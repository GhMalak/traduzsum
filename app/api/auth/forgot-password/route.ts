  ...
  try {
    ...
    if (user) {
      try {
        ...
        console.log(`✅ Token de recuperação gerado e salvo para: ${email}`)
        ...
        try {
          ...
          console.error('📧 Configurações SMTP verificadas:', {
            ...
            siteUrl: process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXT_PUBLIC_BASE_URL || 'NÃO CONFIGURADO',
          })
        ...
        console.error('❌ Erro ao gerar/salvar token de recuperação:', tokenError)
        // Continuar mesmo se falhar para não revelar se o email existe
      }
    }
